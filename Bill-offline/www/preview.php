...
// LOGIC FOR PREVIEWING AN UNSAVED INVOICE (from Session/POST)
} else {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['product_id'])) {
        $customer_id = intval($_POST['customer_id']);
        $invoice_date = $_POST['invoice_date'] ?? date('Y-m-d');
        $invoice_type = $_POST['invoice_type'] ?? 'Invoice';
        $with_tax_value = $_POST['with_tax'] ?? 'Yes';

        $invoice_items = [];
        for ($i = 0; $i < count($_POST['product_id']); $i++) {
            // --- MODIFICATION START ---
            $input_tax_percent = isset($_POST['cgst'][$i]) ? floatval($_POST['cgst'][$i]) : 0;
            $tax_type = 'C'; // Default to 'C' for CGST/SGST

            // Determine the tax type based on the input percentage
            if ($input_tax_percent == 18 || $input_tax_percent == 28) {
                $tax_type = 'I'; // Use 'I' for ICGST/ISGST
            }

            // The CGST and SGST percentages are the same as the input value. NO SPLITTING.
            ...
            $invoice_items[] = [
                ...
                'discount' => isset($_POST['discount'][$i]) ? floatval($_POST['discount'][$i]) : 0,
                'cgst' => $cgst_percent,
                'sgst' => $sgst_percent,
                'tax_type' => $tax_type // Store the type for display logic
            ];
        }
        
        $_SESSION['invoice_data'][$customer_id] = [
            'date' => $invoice_date, 'invoice_type' => $invoice_type, 'with_tax' => $with_tax_value, 'items' => $invoice_items
        ];
    ...
    if (isset($customer_id)) {
        $session_data = $_SESSION['invoice_data'][$customer_id] ?? [];
        $display_items = $session_data['items'] ?? [];
        $display_date = $session_data['date'] ?? date('Y-m-d');
        $display_invoice_type = $session_data['invoice_type'] ?? 'Invoice';
        $display_with_tax = $session_data['with_tax'] ?? 'Yes';
...
foreach ($display_items as $item) {
    ...
    if ($with_tax) {
        ...
        // --- MODIFICATION: Check for I-type tax to change overall labels ---
...
<head>
  ...
  <style>
    ...
    @media print {
        ...
        .invoice-container { margin: 1.5cm; padding: 0; box-shadow: none; border: none; width: auto; }
...
<div class="invoice-container">
    <?php if ($customer && !empty($item_pages)): ?>
        <?php foreach ($item_pages as $page_index => $page_items): ?>
            <?php
            // Logic to aggregate tax breakdown per page
            ...
            foreach ($page_items as $item) {
                ...
                if ($with_tax) {
                    ...
                    $tax_type = $item['tax_type'] ?? 'C'; // Default to 'C' for saved invoices
        ...
        <div class="invoice-page">
            ...
            <div class="col-4 text-end invoice-details">
                ...
                <div><strong>Invoice :</strong> <?= $is_saved_view ? htmlspecialchars($invoice['invoice_number']) : '[Pending]' ?></div>
                <?php if ($total_pages > 1): ?>
                <div><strong>Page:</strong> <?= $page_index + 1 ?> of <?= $total_pages ?></div>
                <?php endif; ?>
            </div>
            </div>
                    ...
                    <div class="col-7">
                        ...
                        if ($with_tax && (!empty($page_tax_breakdown))) {
                            ...
                            foreach ($page_tax_breakdown as $data) {
                                // Determine labels based on the stored 'type'
                    ...
                    <div class="col-sm-7 col-md-6 col-lg-5">
                        <table class="table table-sm totals-table">
                            <tbody>
                                ...
                                <?php if ($with_tax): ?>
                                <?php
                                    // --- MODIFICATION: Set overall tax labels based on the flag ---
...
<div class="modal fade" id="successModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header bg-success text-white"><h5 class="modal-title">Success!</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body text-center"><i class="fas fa-check-circle fa-4x text-success mb-3"></i><p class="fs-5">The invoice has been saved successfully!</p></div><div class="modal-footer justify-content-center"><a href="preview.php" id="viewInvoiceBtn" class="btn btn-primary">View Saved Invoice</a><a href="step-1.php" class="btn btn-secondary">Create New Invoice</a></div></div></div></div>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.7/dist/js/bootstrap.bundle.min.js"></script>
<script>
document.addEventListener('DOMContentLoaded', function() { const urlParams = new URLSearchParams(window.location.search); if (urlParams.get('status') === 'success') { const invoiceId = urlParams.get('invoice_id'); const viewBtn = document.getElementById('viewInvoiceBtn'); if (invoiceId && viewBtn) { viewBtn.href = 'preview.php?id=' + invoiceId; } const successModal = new bootstrap.Modal(document.getElementById('successModal')); successModal.show(); } });
</script>
</body>
</html>